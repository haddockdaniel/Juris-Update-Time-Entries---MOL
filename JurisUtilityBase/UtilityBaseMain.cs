...
//            listBoxCompanies.SelectedIndexChanged += listBoxCompanies_SelectedIndexChanged;
        ...
        private static bool IsNumeric(string test)
        {
            ...
            {
                MessageBox.Show("That is not a valid Entry ID", "Entry Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
        ...
        {
            string SQL = "update[BilledTime] set btcode1 = '0', btcode2 = '0' " +
                        " from billedtime j " +
                        " inner join employee e on e.empsysnbr = j.bttkpr " +
                        " where left(btcode1, 3) = empinitials and btdate >= '01/01/2019' ";
            _jurisUtility.ExecuteNonQuery(0, SQL);

            SQL = "update[unBilledTime] set utcode1 = '0', utcode2 = '0' " +
                        " from unbilledtime j " +
                        " inner join employee e on e.empsysnbr = j.uttkpr " +
                        " where left(utcode1, 3) = empinitials and utdate >= '01/01/2019' ";
            _jurisUtility.ExecuteNonQuery(0, SQL);

            MessageBox.Show("Successfully Bulk Updated all time entries", "Success!", MessageBoxButtons.OK, MessageBoxIcon.None);
        }
    }
}
